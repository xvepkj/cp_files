...
int main() { 
    ...
    bool isforest=true;
    for(int i=0;i<m;i++)if(ispart[i]==false)isforest=false;
    if(isforest)printf("forest");
    else {
        int i;
        for(i=0;i<m;i++)if(ispart[i]==false){printf("%d",dax[i].first);break;}
        for(int j=i+1;j<m;j++)if(ispart[j]==false)printf(" %d",dax[j].first);
    }
    printf("\n");
}
}
